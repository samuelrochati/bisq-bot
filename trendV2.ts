...
async function handlePositionManagement(
  ...
  if (pnlUSDT >= TP_USDT) {
    ...
    return { closed: true, pnl: pnlUSDT };
  }
    ...
    printStatus({
      ...
      openOrder:
        isNoPosition(pos) && lastOpenOrder.side && lastOpenOrder.price
          ? {
              side: lastOpenOrder.side,
              price: lastOpenOrder.price,
              amount: TRADE_AMOUNT,
            }
          : null,
      closeOrder:
        !isNoPosition(pos) && lastCloseOrder.side && lastCloseOrder.price
          ? {
              side: lastCloseOrder.side,
              price: lastCloseOrder.price,
